...
def camel_case_split(identifier):
    return (
        re.sub(r"([A-Z][a-z])", r" \1", re.sub(r"([A-Z]+)", r" \1", identifier))
        .strip()
        .split()
    )
...
def tokenize_identifier(token, with_con=False):
    if with_con:
        id_tokens = " <con> ".join(
            tokenize_identifier_raw(token, keep_underscore=True)
        ).split()
    else:
        id_tokens = [
            t.lower() for t in tokenize_identifier_raw(token, keep_underscore=False)
        ]
    return id_tokens
...
def tokenize_text(text):
    ...
    content_tokens = re.sub(
        r'([-!"#$%&\'()*+,./:;<=>?@\[\\\]^`{|}~])', r" \1 ", nltk_tokenized
    ).split()
