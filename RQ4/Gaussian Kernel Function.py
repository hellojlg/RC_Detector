...
class HGTConv(MessagePassing):
    ...
    def __init__(
        ...
        # self.k_rel(乘矩阵）和self.v_rel（异质消息）对于每个边的每个头都有一个独立的参数，有num_types个
    ...
    def message(self, k_j: Tensor, q_i: Tensor, v_j: Tensor, edge_attr: Tensor,
        ...
        sigma = 1.0
        distance = torch.norm(q_i - k_j, p=2, dim=-1)  # 计算 q_i 和 k_j 的欧几里得距离
        alpha = torch.exp(-distance ** 2 / (2 * sigma ** 2))  # 应用高斯核函数
        alpha = alpha * edge_attr  # 将 edge_attr 乘入注意力权重
        alpha = softmax(alpha, index, ptr, size_i)  # 应用 softmax 进行归一化
        out = v_j * alpha.view(-1, self.heads, 1)  # 按注意力权重加权 value
