...
# 创建解析器
parser = argparse.ArgumentParser(description="Choose an attention to run")
parser.add_argument(
    "--attention",
    choices=["Additive Attention","Cosine Similarity Attention","GAT", "Gaussian Kernel Function"],  # 列出所有可能的 submodel 名称
    required=True,
    help="Specify the attention file to run (e.g.,GAT, Gaussian Kernel Function)"
)

args = parser.parse_args()

# 调用 model.py 中的函数，传递 submodel 名称
run_model(args.attention)
...
def init_model(device, metadata):
    ...
    hgtModel = HGT(device, 768, 768, 768 * 2, metadata=metadata, heads=8)
    hgtModel = hgtModel.to(device)
    rankNetModel = rankNet(768 * 2)
    rankNetModel = rankNetModel.to(device)

    optimizer = torch.optim.Adam(
        chain(hgtModel.parameters(), rankNetModel.parameters()), lr=0.000005, weight_decay=0.001
    )
