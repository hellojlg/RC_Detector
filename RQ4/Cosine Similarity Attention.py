...
class HGTConv(MessagePassing):
    ...
    def message(self, k_j: Tensor, q_i: Tensor, v_j: Tensor, edge_attr: Tensor,
        ...
        q_i_norm = torch.nn.functional.normalize(q_i, p=2, dim=-1)
        k_j_norm = torch.nn.functional.normalize(k_j, p=2, dim=-1)
        alpha = torch.sum(q_i_norm * k_j_norm, dim=-1) * edge_attr
