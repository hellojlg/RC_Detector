...
parser = argparse.ArgumentParser(description="Specify RNN type")
parser.add_argument(
    "--rnn_type",
    choices=["gru","lstm","transformer","gru1", "gru2", "gru3"],
    required=True,
    help="Specify the RNN type for the model (gru,lstm,transformer,gru1, gru2, or gru3)"
)
...
def init_model(device, metadata):
    ...
    hgtModel = HGT(device, 768, 768, 768 * 2, metadata=metadata, heads=8,rnn_type=args.rnn_type)
