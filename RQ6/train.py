...
parser = argparse.ArgumentParser(description=" Different Embedding Layer")
parser.add_argument(
    "--model_type",
    choices=["graphcodebert", "codet5", "unixcoder", "codebert"],
    required=True,
    help="Specify the model type to use (e.g., graphcodebert, codet5, unixcoder, codebert)"
)
...
def init_model(device, metadata):
    ...
    hgtModel = HGT(device, 768, 768, 768 * 2, metadata=metadata, model_type=args.model_type)
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        print("No CUDA.")
    genAllMiniGraphs(args.model_type)
